...
export const newsArticles: NewsArticle[] = [
  {
    ...
    faq: [
      {
        question: 'NoahAI 전략 스튜디오는 무엇인가요?',
        answer:
          'Pine Script, TradingView 아이디어, 문서와 자연어로 된 투자전략을 실행 가능한 규칙으로 구조화하고, 과거검증과 PAPER 전진검증, 버전 관리, 전략 공유와 운용까지 연결하는 NoahAI의 전략 작업 공간이다. 기존 AI 커스텀의 공식 사용자 명칭을 전략 스튜디오로 정식화했다.',
      },
      {
        question: '기존 백테스트 서비스와 무엇이 다른가요?',
        answer:
      ...
      },
      {
        ...
        answer:
          '선택한 전략을 자신의 거래소와 비용 조건에서 다시 검증하고 PAPER로 확인한 뒤 승인할 수 있다. NoahAI는 현재 시장국면과 전략의 적합성을 판단해 맞는 전략을 운용 후보로 검토하고, 결과를 같은 전략 버전의 검증 근거로 다시 축적한다.',
      },
      {
        question: '왜 전략 시장의 게임체인저라고 하나요?',
        answer:
          '전략을 판매자의 수익률 화면으로 평가하던 시장을 구매자가 같은 전략 버전의 근거를 확인하고 자신의 환경에서 다시 검증하는 시장으로 바꾸기 때문이다. 제작자는 검증 품질로 경쟁하고 사용자는 자신에게 맞는 전략을 더 투명하게 선택할 수 있다.',
      },
      {
        question: '가짜 수익률이나 사기 전략을 어떻게 줄이나요?',
        answer:
          '유리한 기간만 선택한 백테스트, 비용을 제외한 수익률, PAPER 결과를 실제 체결처럼 제시한 화면, 다른 버전의 성과를 구분해 표시한다. 수익률 주장만으로 전략을 평가하지 않고 재현 가능한 검증 근거를 함께 보게 만드는 방식이다.',
      },
      {
        question: 'NoahAI가 말하는 세계 최초의 범위는 무엇인가요?',
        answer:
          '백테스트나 전략 판매 같은 개별 기능이 아니라 전략 원문, 실행 규칙, 시장국면별 검증, 거래소별 PAPER와 확인 체결, 버전별 검증 여권, 구매자 재검증과 마켓 탐색을 하나의 생명주기로 연결한 결합 구조를 뜻한다. 세계 최초 표현은 2026년 9월 NoahAI Labs 자체 비교조사 기준이다.',
      },
    ],
    content: `
      ...
      <p class="text-lg text-gray-800 mb-6 leading-relaxed">
        금융 AI 플랫폼 <strong>NoahAI</strong>를 운영하는 <strong>노아에이아이랩스(NoahAI Labs)</strong>가 투자전략의 제작과 검증, 유통, 운용을 하나로 연결하는 <strong>&lsquo;전략 검증 여권(Strategy Passport)&rsquo; 생태계</strong>를 공개했다고 밝혔다. 기존 &lsquo;AI 커스텀&rsquo; 기능의 공식 명칭도 <strong>&lsquo;NoahAI 전략 스튜디오(Strategy Studio)&rsquo;</strong>로 정식화했다.
      </p>

      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        그동안 온라인 투자전략 시장에서는 백테스트 수익률이나 계좌 화면이 전략의 성능을 설명하는 대표 자료로 활용돼 왔다. 그러나 유리한 기간만 선택했는지, 수수료와 슬리피지가 반영됐는지, 판매된 전략과 성과를 낸 전략이 같은 버전인지 사용자가 확인하기는 어려웠다. 상승장에서 성과가 좋았던 전략이 횡보장이나 급락장에서는 어떻게 작동하는지도 하나의 평균 수익률만으로는 알기 어려웠다.
      </p>

      <figure class="mb-10 overflow-hidden rounded-2xl border border-slate-200 bg-slate-950">
        <img src="/images/news/news-96-noahai-strategy-studio-passport-og.jpg" alt="전략 원문부터 실행 규칙 PAPER 검증 전략 검증 여권과 사용자 재검증까지 연결하는 NoahAI 전략 스튜디오" width="1200" height="630" class="h-auto w-full" loading="eager" decoding="async" />
        <figcaption class="px-5 py-3 text-sm text-slate-300">전략 원문과 실행 규칙, 검증 결과를 하나의 여권으로 연결하는 NoahAI 전략 스튜디오.</figcaption>
      </figure>
      ...
      <p class="text-lg text-gray-700 mb-5 leading-relaxed">
        전략마다 발급되는 검증 여권에는 다음 정보가 함께 기록된다.
      </p>
      <ul class="list-disc pl-6 text-lg text-gray-700 mb-8 space-y-3 leading-relaxed">
        <li><strong>전략 원문과 실행 규칙:</strong> 어떤 아이디어가 어떤 조건으로 실제 실행되는지 확인</li>
        <li><strong>전략 ID와 버전:</strong> 판매·검증·운용된 전략이 동일한 버전인지 구분</li>
        <li><strong>시장과 비용 조건:</strong> 거래소, 자산, 현물·선물, 시간봉, 수수료와 슬리피지 기록</li>
        <li><strong>검증 결과:</strong> 과거검증, 시장국면별 성과, 모의 운용(PAPER)과 확인된 거래 근거를 단계별로 표시</li>
      </ul>
      ...
      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        마켓에서 선택한 전략은 NoahAI 전략 스튜디오로 가져와 자신의 거래소와 자산, 수수료 조건에서 다시 검증하고 PAPER로 확인한 뒤 승인할 수 있다. NoahAI와 연동하면 현재 시장국면과 승인된 전략의 적합성을 함께 판단해 전략에 유리한 국면에서는 운용 후보로 연결하고, 국면이 바뀌면 다른 승인 전략이나 NoahAI의 기본 판단으로 전환할 수 있다.
      </p>
      ...
      <figure class="mb-10 overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <img src="/images/news/news-89-noahai-ai-custom-loop.jpg" alt="전략 학습과 제작 검증 승인 운용 결과 검토가 반복되는 NoahAI 전략 스튜디오 생명주기" width="1200" height="630" class="h-auto w-full" loading="lazy" decoding="async" />
        <figcaption class="px-5 py-3 text-sm text-slate-600">전략 제작부터 검증, 승인, 운용, 결과 검토까지 반복되는 NoahAI의 전략 생명주기.</figcaption>
      </figure>
      ...
      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        이 구조가 정착하면 유리한 기간만 고른 백테스트, 비용을 제외한 수익률, 모의 결과를 실제 성과처럼 제시한 화면이나 다른 버전의 성과를 가져온 광고는 높은 숫자 하나만으로 신뢰를 얻기 어려워진다. 마켓 역시 다운로드 수나 자기신고 수익률이 아니라 표본, 최근성, 시장국면별 적합성, 위험과 실행 근거를 중심으로 전략을 보여줄 수 있다.
      </p>

      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        회사 측은 Pine·TradingView·문서 전략의 원문과 실행 규칙을 연결하고, 시장국면별 검증과 거래 근거를 버전별 여권으로 축적하며, 사용자가 자신의 환경에서 재검증해 NoahAI 운용까지 이어가는 전체 구조가 <strong>2026년 9월 자체 글로벌 주요 서비스 비교조사 기준 세계 최초</strong>라고 설명했다. 백테스트나 전략 판매라는 개별 기능이 아니라 전략의 제작부터 검증·유통·운용까지 하나의 생명주기로 연결한 것이 핵심이다.
      </p>

      <p class="text-lg text-gray-800 mb-8 leading-relaxed border-l-4 border-primary-600 pl-5">
        노아에이아이랩스는 &ldquo;좋은 전략의 기준은 가장 높은 수익률을 보여주는 화면이 아니라, 어떤 시장에서 어떻게 작동했고 누구나 같은 조건으로 다시 확인할 수 있는가에 있다&rdquo;며 &ldquo;전략 스튜디오를 통해 제작자는 실력과 근거로 인정받고, 사용자는 자신의 시장과 위험 수준에 맞는 전략을 선택하는 새로운 전략 문화를 만들겠다&rdquo;고 전했다.
      </p>

      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        노아에이아이랩스는 향후 검증 전략 허브를 기반으로 전략 등록과 검토, 사용자 재검증 체계를 확대하고 전략 라이선스와 제작자 보상까지 연결되는 마켓플레이스로 발전시킬 계획이다. 자세한 내용은 <a href="https://noahailabs.com/ko/product/strategy-studio" target="_blank" rel="noopener noreferrer" class="text-primary-600 underline">NoahAI 전략 스튜디오 공식 페이지</a>와 <a href="https://daltrading.net/strategies" target="_blank" rel="noopener noreferrer" class="text-primary-600 underline">검증 전략 허브</a>에서 확인할 수 있다.
      </p>

      <p class="text-gray-600 text-sm border-t border-gray-200 pt-6 leading-relaxed">
        ※ 전략 검증 결과는 투자 판단을 돕기 위한 정보이며 미래 수익을 보장하지 않습니다.
      </p>
    `,
  },
  {
    id: 95,
    title:
      '자람이, 국내 첫 ‘디지털케어로그 연계형 JDS 시각언어 규격’ 공개',
    excerpt:
      '가정·학교·센터에서 같은 의미를 사용하도록 의미 ID와 L1~L3 개인별 표현, Core 48 인쇄 자료, AI 디지털케어로그 적용 규칙을 하나로 연결한 발달지원 시각언어 규격.',
    category: 'press',
    date: '2026-09-04',
    modifiedDate: '2026-09-06',
    image: '🗣️',
    ...
    contentImages: [
      ...
      },
      {
        ...
        caption:
          'JDS Core 48 실제 화면에서 요구와 자기결정에 필요한 시각카드를 선택하고 인쇄할 수 있다.',
      },
    ],
    about: [
      {
    ...
    faq: [
      {
        question: 'JDS는 무엇인가요?',
        answer:
          'JDS는 자람이가 제공하는 발달지원 시각언어 규격이다. 자람이 서비스 화면, Jaram Visual Language System, 학습 템플릿과 AI 디지털케어로그 적용 규칙을 Meaning ID 중심의 하나의 의미체계로 연결한다.',
      },
      {
        question: 'L1~L3 표현 단계는 어떻게 다른가요?',
        answer:
          'L1은 실물과 실제 사진, L2는 JDS 표준 시각자료, L3는 단순 기호와 글자를 사용한다. 표현 형태가 달라도 같은 Meaning ID를 유지하며, 진단명이나 나이가 아니라 당사자가 가장 잘 이해하는 방식을 기준으로 선택한다.',
      },
      {
        question: 'Core 48 시각자료는 어디에서 출력하나요?',
        answer:
          '자람이 JDS 공식 페이지에서 Core 48 카드를 선택해 낱장 카드, 먼저-그다음 보드, 선택판, 시각일정표 형태로 A4 인쇄하거나 PDF로 저장할 수 있다.',
      },
      {
        question: 'Essential 150도 현재 그림카드로 사용할 수 있나요?',
        answer:
          'Core 48은 승인된 시각자료로 공개됐다. Essential 150의 추가 102개 의미는 현재 현장 검토 단계이며 승인된 그림 자산이 아니다.',
      },
      {
        question: '국내 첫 시도라는 표현은 어떤 범위인가요?',
        answer:
          'AAC나 그림상징 자체의 최초라는 뜻이 아니다. 한국어 Meaning ID, L1~L3 개인별 표현, Core 48 인쇄 자료, 자람이 제품 UI·시각언어·학습 템플릿, AI 디지털케어로그 적용 규칙을 하나의 규격으로 결합한 구조를 뜻한다.',
      },
    ],
    content: `
      <p class="text-sm text-gray-500 mb-6">[보도자료 · 2026-09-04]</p>
      <p class="text-xl font-semibold text-gray-800 mb-3 leading-relaxed">— 가정·센터·학교에서 같은 의미 쓰는 발달지원 시각언어 표준 제시</p>
      <p class="text-xl font-semibold text-gray-800 mb-8 leading-relaxed">— 의미 ID 중심 구조로 개인별 이해 수준(L1~L3)에 맞춘 유연한 시각표현 지원</p>

      <p class="text-lg font-semibold text-gray-800 mb-8 leading-relaxed border-l-4 border-primary-600 pl-4">
        발달장애 통합지원 플랫폼 <strong>자람이</strong>가 발달장애인의 의사소통과 일상생활 지원을 위한 <strong>JDS(Jaram Design System) 발달지원 시각언어 규격</strong>을 공개했다고 밝혔다. 이번 JDS는 자람이 서비스 화면, Jaram Visual Language System(JVLS), 학습 템플릿과 AI 디지털케어로그 적용 규칙을 하나의 통합 의미체계로 연결한 것이 특징이다.
      </p>
      <figure class="mb-10 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
        ...
        <figcaption class="px-5 py-3 text-sm text-slate-500">JDS는 같은 의미를 여러 생활환경에서 일관되게 사용하되, 표현 방식은 개인의 이해 특성에 맞게 선택하도록 설계됐다.</figcaption>
      </figure>

      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        발달장애인은 가정, 학교, 치료센터, 병원 등 다양한 환경을 오가며 생활하지만 장소마다 사용하는 사진과 그림, 기호가 다르면 같은 뜻도 매번 새로 이해해야 하는 어려움이 생길 수 있다. JDS는 <strong>&ldquo;같은 의미를, 어디서나 같은 시각언어로&rdquo;</strong>라는 원칙 아래 당사자가 환경마다 새로운 표현법에 맞추기보다, 주변 환경이 일관된 의미체계를 공유하도록 설계됐다.
      </p>
      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        핵심은 특정 그림을 고정하는 대신 <strong>&lsquo;의미 ID(Meaning ID)&rsquo;</strong>를 중심에 두는 것이다. 예를 들어 &lsquo;물을 원한다&rsquo;는 의미에 <code>need.water</code>라는 ID를 부여한 뒤, 당사자의 언어·인지·감각 특성에 맞춰 다음과 같은 3단계 표현으로 제공한다.
      </p>
      <ul class="list-disc pl-6 text-lg text-gray-700 mb-8 space-y-2 leading-relaxed">
        <li><strong>L1(실물·실제 사진)</strong>: 본인의 컵, 교실, 가족 등 높은 구체성이 필요한 사용자 단계</li>
        <li><strong>L2(JDS 표준 시각자료)</strong>: 일관된 선·색·구도와 한 장에 한 의미를 사용하는 기본 단계</li>
        <li><strong>L3(기호·단순 문자)</strong>: 기호와 짧은 글자를 이해하고 빠르게 탐색할 수 있는 사용자 단계</li>
      </ul>
      <figure class="mb-10 overflow-hidden rounded-2xl border border-teal-100 bg-slate-50">
        ...
        <figcaption class="px-5 py-3 text-sm leading-relaxed text-slate-600">동일한 <code>need.water</code> 의미 ID를 유지하면서 사용자에 따라 L1 실물·사진, L2 JDS 표준 시각자료, L3 기호·글자로 표현한다.</figcaption>
      </figure>

      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        첫 공개 세트인 <strong>Core 48</strong>은 국제적으로 정해진 48개 표준어휘가 아니라 요구·자기결정, 안전·신체, 감정, 기본생활, 사람·장소 등 발달지원 현장에서 우선 필요한 의미를 자람이가 구성한 시작 어휘체계다. 보호자와 특수교사, 치료사는 자람이 JDS 페이지에서 필요한 카드를 고르고, 낱장 카드·먼저–그다음 보드·선택판·시각일정표 형태로 A4 인쇄하거나 PDF로 저장해 활용할 수 있다.
      </p>
      <figure class="mb-10 overflow-hidden rounded-2xl border border-teal-100 bg-slate-50">
        ...
        <figcaption class="px-5 py-3 text-sm leading-relaxed text-slate-600">자람이 JDS Core 48 실제 화면. 요구·자기결정 범주의 시각카드를 선택해 일상에서 바로 사용할 수 있다.</figcaption>
      </figure>

      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        JDS는 시각자료의 활용 맥락을 <a href="https://jarame.or.kr/tech-docs" target="_blank" rel="noopener noreferrer" class="text-primary-600 underline">자람이 AI 디지털케어로그</a>의 관찰·지원 흐름과 연결한다. 수업 중 자리이탈과 같은 행동을 단순히 횟수로 남기는 데서 그치지 않고, <strong>환경 → 당사자 표현 → 제공한 지원 → 관찰 반응 → 조정</strong>의 흐름을 기록해 &lsquo;시끄러워요&rsquo;, &lsquo;쉬고 싶어요&rsquo;와 같은 당사자의 표현 맥락을 파악하도록 돕는 구조다.
      </p>
      <p class="text-lg text-gray-700 mb-6 leading-relaxed">
        드림에이아이랩 <strong>정해성 CTO</strong>와 대화분석·상호작용 언어학 전문가 <strong>강수진 박사</strong>가 공동 설계한 JDS는 한국어 의미 ID, L1~L3 개인별 표현, Core 48 인쇄 자료, 자람이 제품 UI·시각언어·학습 템플릿, AI 디지털케어로그 적용 규칙을 하나의 규격으로 결합한 구조 측면에서 <strong>국내 첫 시도</strong>다.
      </p>
      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        정해성 CTO는 &ldquo;표현하지 못한 의사가 행동으로 나타나는 것은 아닌지 먼저 살펴볼 수 있어야 한다&rdquo;며 &ldquo;당사자가 환경마다 새로운 표현법에 맞추기보다, 주변 환경이 일관된 의미를 사용하도록 만드는 것이 JDS의 출발점&rdquo;이라고 말했다.
      </p>
      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        자람이는 향후 Core 48을 기반으로 어휘 체계를 확장하고, 가정·학교·치료·지역사회 환경에서 활용할 수 있는 표현과 Personal Vocabulary를 단계적으로 고도화할 계획이다. Essential 150의 추가 102개 의미는 현재 현장 검토 단계로, 승인된 그림 자산은 Core 48에 한해 공개됐다.
      </p>
      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        JDS 상세 내용 확인과 Core 48 시각자료 선택·출력은 <a href="https://jarame.or.kr/design-system" target="_blank" rel="noopener noreferrer" class="text-primary-600 underline">자람이 JDS 공식 페이지</a>에서 이용할 수 있다.
      </p>
      <p class="text-gray-600 text-sm border-t border-gray-200 pt-6 leading-relaxed">
        ※ JDS v1은 발달지원 시각언어 규격으로, 진단·처방·치료나 전문가의 최종 판단을 대체하지 않는다. 시각자료를 사용하기 전에 당사자가 각 그림을 의도한 의미로 이해하는지 확인해야 한다.
      </p>
    `,
  },
  {
  ...
  },
  {
    ...
    ogImageAlt:
      '재가돌봄·주간보호에서 요양원·요양병원까지 이어지는 시니어앤라이프 돌봄 현장',
    ogImageAltEn:
      'Senior & Life connecting home care, day care, nursing homes and hospitals',
    about: [
      {
        type: 'Organization',
    ...
    faq: [
      ...
      },
      {
        ...
        answer:
          '최근 6개월간 복약·식사·수면·활동량·인지 상태·낙상 위험 등 시니어 일상 데이터를 수집·품질 처리한 규모다. 하루 평균 약 4만 9,000건이다.',
      },
      {
        question: '요양원·요양병원에도 들어가나?',
        answer:
          '재가·주간보호에서 확보한 점유율을 바탕으로 요양원과 요양병원으로 서비스를 본격 확장한다. 목표는 한 사람의 노년기를 잇는 생애 통합 돌봄 OS다.',
      },
      {
        question: 'AI 디지털케어로그는 무엇인가?',
        answer:
          '돌봄 환경이 바뀌어도 한 사람을 중심으로 장기간 상태 변화를 연결·추적하는 시니어앤라이프의 돌봄 데이터 구조다. 시설 관리 프로그램을 넘어 생애 돌봄 인프라를 지향한다.',
      },
    ],
    i18n: {
      en: {
        ...
        faq: [
          ...
          },
          {
            ...
            answer:
              'Six months of quality-processed senior daily data — medication, meals, sleep, activity, cognition, and fall risk — about 49,000 records a day.',
          },
          {
            question: 'Is it expanding into nursing homes and hospitals?',
            answer:
              'Yes. On the home-care and day-care base, Senior & Life is expanding into nursing homes and long-term hospitals to build a life-course care OS.',
          },
          {
            question: 'What is the AI Digital Care Log?',
            answer:
              'A person-centered care-data structure that keeps records when the setting changes, from home care to day care, nursing homes, and hospitals.',
          },
        ],
        content: `
  ...
  {
    id: 84,
    title: '노아AI v3.9.0.2, TradingView·Pine 전략 자동화와 시장국면별 안전 운용 공개',
    excerpt:
      'NoahAI Labs가 사용자 제공 TradingView·Pine 전략을 설명 가능한 규칙으로 구조화하고, 시장국면에 따라 승인 전략을 선택·중단하는 AI 커스텀 v3.9.0.2의 소스·문서 기준을 공개했습니다.',
    category: 'updates',
    date: '2026-07-26',
    image: '🧠',
    featured: true,
    ...
    i18n: {
      en: {
        title: 'NoahAI v3.9.0.2: TradingView/Pine strategy automation with regime-aware safety',
        excerpt:
          'NoahAI Labs published the v3.9.0.2 source-and-documentation baseline for structuring user-provided TradingView and Pine strategy material into explainable rules and selecting or pausing approved strategies by market regime.',
        content: `
          <p class="text-sm text-gray-500 mb-6">[Technology update · 2026-07-26]</p>
          <p class="text-lg font-semibold text-gray-800 mb-8 leading-relaxed border-l-4 border-primary-600 pl-4">
            <strong>NoahAI Labs</strong> published the NoahAI Client v3.9.0.2 source-and-documentation baseline.
            AI Custom can structure user-provided Pine Script, public TradingView strategy descriptions, text, PDF, OCR, and video material into reviewable rules.
          </p>
          <h2 class="text-2xl font-bold text-gray-900 mb-4">Useful to TradingView users, without claiming replacement</h2>
          <p class="text-lg text-gray-700 mb-8 leading-relaxed">
            TradingView remains strong in charts, Pine authoring, backtesting, community scripts, and broker integrations.
            NoahAI adds a complementary operating layer: source evidence, missing-condition questions, user approval, market-regime fit, runtime guardrails, and rollback.
            Protected scripts are not bypassed.
          </p>
          <h2 class="text-2xl font-bold text-gray-900 mb-4">Strategy selection when markets change</h2>
          <p class="text-lg text-gray-700 mb-8 leading-relaxed">
            Among approved strategies, only those matching the current asset, exchange, and declared regime become new-entry candidates.
            A regime mismatch can delegate to the default NoahAI path or pause new custom entries. Entry conditions and guardrails still take precedence.
          </p>
          <h2 class="text-2xl font-bold text-gray-900 mb-4">KPI improvement is published with cost boundaries</h2>
          <p class="text-lg text-gray-700 mb-8 leading-relaxed">
            A dated July 24 internal analysis snapshot showed a 50.98% seven-day win rate across 306 closed trades,
            +0.056 USDT before fees and an estimated -0.730 USDT after fees. Monthly estimated fee-inclusive losses narrowed,
            but the result remains negative and does not prove profitability.
          </p>
          <p class="text-sm text-gray-500">
            Canonical product details: <a href="https://noahailabs.com/en/blog/research/ai-custom-financial-automation" class="text-primary-600 underline">NoahAI Labs</a>.
            This Dream AI Lab article is an independent cross-domain technology update, not third-party validation or investment advice.
          </p>
        `,
      },
    },
    content: `
      <p class="text-sm text-gray-500 mb-6">[기술 업데이트 · 2026-07-26]</p>
      <p class="text-lg font-semibold text-gray-800 mb-8 leading-relaxed border-l-4 border-primary-600 pl-4">
        <strong>NoahAI Labs</strong>가 NoahAI Client v3.9.0.2 소스·문서 기준을 공개했습니다.
        AI 커스텀은 사용자가 제공한 Pine Script, 공개 TradingView 전략 설명, 텍스트, PDF, OCR, 영상 자료를 검토 가능한 규칙으로 구조화합니다.
      </p>
      <h2 class="text-2xl font-bold text-gray-900 mb-4">TradingView 사용자를 보완하는 운영 계층</h2>
      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        TradingView는 차트, Pine 작성·백테스트, 커뮤니티, 브로커 연동에서 강합니다.
        NoahAI는 원문 근거, 누락 조건 질문, 사용자 승인, 시장국면 적합성, 실행 가드레일, 롤백을 연결합니다.
        보호된 스크립트를 우회하거나 TradingView 전체를 대체한다고 주장하지 않습니다.
      </p>
      <h2 class="text-2xl font-bold text-gray-900 mb-4">시장이 바뀌면 승인 전략을 선택하거나 중단</h2>
      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        승인된 전략 중 현재 자산·거래소·시장국면이 맞는 전략만 신규 진입 후보가 됩니다.
        국면이 맞지 않으면 기본 NoahAI 판단에 위임하거나 커스텀 신규 진입을 일시정지하며,
        진입조건과 공통 가드레일을 추가로 통과해야 합니다.
      </p>
      <h2 class="text-2xl font-bold text-gray-900 mb-4">KPI 개선은 비용 경계와 함께 공개</h2>
      <p class="text-lg text-gray-700 mb-8 leading-relaxed">
        2026년 7월 24일 내부 분석 스냅샷은 최근 7일 종료 거래 306건, 승률 50.98%,
        수수료 전 +0.056 USDT, 추정 수수료 반영 후 -0.730 USDT를 기록했습니다.
        월별 추정 수수료 포함 손실 폭은 줄었지만 아직 음수이며, 수익성 입증이나 미래 성과 보장을 의미하지 않습니다.
      </p>
      <p class="text-sm text-gray-500">
        제품 정본: <a href="https://noahailabs.com/ko/blog/research/ai-custom-financial-automation" class="text-primary-600 underline">NoahAI Labs AI 커스텀 기술 해설</a>.
        이 글은 드림에이아이랩의 독립적인 교차 도메인 기술 업데이트이며 제3자 검증이나 투자 권유가 아닙니다.
      </p>
    `,
  },
  {
