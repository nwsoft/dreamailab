...
기록이 끊기지 않을 때 가족·학교·치료 현장이 함께 성장할 수 있다는 믿음에서 출발한, 발달장애 영역의 AI디지털케어로그 인프라(DAL 1번 도메인).

## 홈 카드와의 정합 (2026-04)

- 홈 요약은 **치료 앱이 아닌 인프라·포용·골든타임** 톤으로 서술. 불릿은 `coreFeatures`와 같은 축(케어로그, AI, 리포트, 자가진단, RBAC, 홈티, 통계)을 한 줄로 압축.
